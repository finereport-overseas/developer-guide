# Custom Controls - Style Text Editor
...
### Widget
This is the parent class for a widget. Usually we should also implement *DataControl* in our own widget.
```java
public class TestWidget extends Widget implements DataControl {
    ...
    /**
     * In DataControl, it returns the type supported by the widget return value.
     * @return
     */
    @Override
...
public class XStyleTextEditor extends XTextEditor {
    ...
    /**
     * Since the widget adds a new property fontColor, we need to rewrite the property pane.
     *
     * @return
     * @throws IntrospectionException
     */
...
```
There is an editor class called *StyleColorAttrEditor* referred to above. As long as the property is not of a basic type, the corresponding editor class needs to be created by ourselves.
```java
...
```
As you can see, it also inherits from the *ParameterWidgetOptionProvider*. The only difference is that the UI setting method *appearanceForWidget* has to return a subclass of *BasicBeanPane*.

I am not going to talk too much about *BasicBeanPane* here. Just use the example below, and you will get the rough idea of how to add the style text editor as a cell widget.
```java
