...
| --- | --- |
| 所属模块 | extra-designer |
| 完整类名 | `com.fr.design.fun.MultiStyleUIConfigProvider` |
| 官方文档 | [查看文档](https://wiki.fanruan.com/display/PD/MultiStyleUIConfigProvider) |
...
在制作模板的过程中，每个用户公司都会逐渐形成自身的一套特有的样式风格，为了方便用户快速配置样式，报表设计器中提供了预定义样式的功能，以便让用户快速配置报表的样式。标准设计器中提供了基本的格式、字体、对齐、边框、背景的预定义。但是样式中还可能包含其他属性可自由定义的。MultiStyleUIConfigProvider和StyleUIConfigProvider接口就是用来提供自定义的样式配置扩展的

![预定义样式列表](./images/multistyle_predefined_styles.png)
![多样式 Demo 界面](./images/multistyle_demo_ui.png)
...
StyleUIConfigProvider从形式上看也是一个插件接口，但是实际上目前已经不能单独使用了。统一通过MultiStyleUIConfigProvider引入。
